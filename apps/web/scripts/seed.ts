...
async function run() {
  ...
  await supabase.from('users').insert({
    id: advisorId,
    email: 'advisor@dhanflow.dev',
    role: 'advisor',
  });

  const clients = Array.from({ length: 5 }).map((_, idx) => ({
    advisor_id: advisorId,
    name: `Client ${idx + 1}`,
    email: `client${idx + 1}@dhanflow.dev`,
    phone: `+91-9000000${idx}`,
    risk_profile: idx % 2 === 0 ? 'Moderate' : 'High',
  }));

  const { data: createdClients, error: clientsError } = await supabase.from('clients').insert(clients).select('id');
  if (clientsError) throw clientsError;

  const selectedClients = (createdClients ?? []).slice(0, 3);

  for (const [index, client] of selectedClients.entries()) {
    const { data: portfolio, error: portfolioError } = await supabase
      .from('portfolios')
      .insert({ client_id: client.id, total_value: 4500000 + index * 500000, currency: 'INR' })
      .select('id')
      .single();
    ...
    await supabase.from('holdings').insert([
      { portfolio_id: portfolio.id, symbol: 'NIFTYBEES', asset_type: 'Equity', quantity: 2000 + index * 200, price: 250 },
      { portfolio_id: portfolio.id, symbol: 'GOLD', asset_type: 'Commodity', quantity: 150 + index * 10, price: 6200 },
      { portfolio_id: portfolio.id, symbol: 'BOND', asset_type: 'Debt', quantity: 1000 + index * 100, price: 110 },
    ]);

    await supabase.from('transactions').insert([
      { portfolio_id: portfolio.id, type: 'Buy', amount: 250000 + index * 10000 },
      { portfolio_id: portfolio.id, type: 'SIP', amount: 50000 + index * 5000 },
    ]);
    ...
    await supabase.from('goals').insert([
      {
        ...
        current_amount: 35000000 + index * 5000000,
        deadline: '2038-12-31',
      },
    ]);
  }

  console.log('Seed complete: 5 clients, 3 portfolios, transactions, and goals created');
}
